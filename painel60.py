    ...
    if data_valuation:
            ...
            if 'Status Preço' in df_valuation.columns:
                ...
                for idx, (ticker_s, msg_s) in enumerate(todos_status):
                    with cols_status[idx % 4]:
                        if msg_s.startswith('✅'):
                            cor_bg, cor_borda = "#f0fff4", "#28a745"
                        elif msg_s.startswith('🔴'):
                            cor_bg, cor_borda = "#fff5f5", "#dc3545"
                        else:
                            cor_bg, cor_borda = "#fffbf0", "#fd7e14"

                        st.markdown(
                            f"<div style='background:{cor_bg};border-left:4px solid {cor_borda};"
                            f"border-radius:6px;padding:8px 12px;margin-bottom:8px;'>"
                            f"<span style='font-weight:700;font-size:13px;'>{ticker_s}</span><br>"
                            f"<span style='font-size:12px;color:#444;'>{msg_s}</span>"
                            f"</div>",
                            unsafe_allow_html=True
                        )
        ...
        with tab4:
            ...
            if not tickers_com_dpa:
                ...
                # Usa st_components.html() em vez de st.markdown() para garantir
                # que window.print() seja chamado no contexto correto da página
                # tanto em ambiente local (Python) quanto no Render.
                # ---------------------------------------------------------------
                st.markdown("---")
                st.markdown("#### 🖨️ Imprimir Projeção")
                st.caption(
                    "Clique no botão abaixo para abrir o diálogo de impressão do navegador. "
                ...
                st_components.html(html_print, height=80)
