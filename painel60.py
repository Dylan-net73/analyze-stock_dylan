            ...
            if not tickers_com_dpa:
                ...
                with col_nome:
                    nome_simulacao = st.text_input(
                        ...
                        key="nome_simulacao",
                        help="Digite um nome para identificar esta simulação no arquivo Excel"
                    )
                with col_btn:
                    st.markdown("<br>", unsafe_allow_html=True)

                data_hora_sim = _agora_sp().strftime('%d/%m/%Y %H:%M:%S')
                nome_arquivo  = f"{nome_simulacao.strip().replace(' ', '_')}_{_agora_sp().strftime('%Y%m%d_%H%M')}.xlsx"

                # Monta aba 1 — dados da simulação por ticker
                ...
                # Gera Excel em memória com openpyxl
                ...
                st.download_button(
                    ...
                    help=f"Salva a simulação '{nome_simulacao}' com data/hora {data_hora_sim}"
                )
